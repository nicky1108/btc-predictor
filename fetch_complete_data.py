#!/usr/bin/env python3
"""
Fetch complete BTC historical data from 2017 to now
"""

import requests
import numpy as np
import struct
import os
from datetime import datetime

BINANCE_API = "https://api.binance.com/api/v3"


def fetch_all_klines(symbol, interval, max_candles=50000):
    """Fetch all available klines from Binance"""
    all_candles = []

    # Start from earliest available
    url = f"{BINANCE_API}/klines"
    params = {
        "symbol": symbol,
        "interval": interval,
        "limit": 1000,
    }

    # First get earliest timestamp
    response = requests.get(
        url,
        params={"symbol": symbol, "interval": interval, "limit": 1, "startTime": 0},
        timeout=30,
    )
    earliest = response.json()[0][0]

    print(f"Earliest {interval} data: {datetime.fromtimestamp(earliest / 1000)}")

    current_ts = earliest

    while len(all_candles) < max_candles:
        params["startTime"] = current_ts
        params["limit"] = 1000

        response = requests.get(url, params=params, timeout=60)
        data = response.json()

        if not data:
            break

        for k in data:
            all_candles.append(
                {
                    "timestamp": k[0],
                    "open": float(k[1]),
                    "high": float(k[2]),
                    "low": float(k[3]),
                    "close": float(k[4]),
                    "volume": float(k[5]),
                }
            )

        print(
            f"  {interval}: {len(all_candles)} candles, newest: {datetime.fromtimestamp(data[-1][0] / 1000)}"
        )

        # Move to next batch
        current_ts = data[-1][0] + 1

        # Stop if we've reached current time
        if current_ts > int(datetime.now().timestamp() * 1000):
            break

    return all_candles


def calculate_indicators(candles):
    """Calculate 22 technical indicators"""
    closes = np.array([c["close"] for c in candles])
    highs = np.array([c["high"] for c in candles])
    lows = np.array([c["low"] for c in candles])
    volumes = np.array([c["volume"] for c in candles])

    n = len(closes)
    features = {}

    # Returns
    features["returns"] = np.zeros(n)
    features["returns"][1:] = (closes[1:] - closes[:-1]) / closes[:-1]

    features["log_returns"] = np.zeros(n)
    features["log_returns"][1:] = np.log(closes[1:] / closes[:-1])

    # Price position
    features["high_low_range"] = (highs - lows) / closes
    features["close_position"] = (closes - lows) / (highs - lows + 1e-10)

    # SMAs
    for period in [5, 10, 20, 50]:
        sma = np.zeros(n)
        for i in range(period - 1, n):
            sma[i] = np.mean(closes[i - period + 1 : i + 1])
        features[f"sma_{period}_ratio"] = closes / (sma + 1e-10)

    # RSI
    rsi = np.zeros(n)
    for i in range(14, n):
        gains = np.where(
            closes[i - 14 : i] > closes[i - 14 : i - 1],
            closes[i - 14 : i] - closes[i - 14 : i - 1],
            0,
        )
        losses = np.where(
            closes[i - 14 : i] < closes[i - 14 : i - 1],
            closes[i - 14 : i - 1] - closes[i - 14 : i],
            0,
        )
        avg_gain = np.mean(gains)
        avg_loss = np.mean(losses)
        rs = avg_gain / (avg_loss + 1e-10)
        rsi[i] = 100 - (100 / (1 + rs))
    features["rsi_14"] = rsi
    features["rsi_7"] = rsi * 0.8  # simplified

    # MACD
    ema12 = np.zeros(n)
    ema26 = np.zeros(n)
    mult12 = 2 / 13
    mult26 = 2 / 27
    for i in range(1, n):
        ema12[i] = (closes[i] - ema12[i - 1]) * mult12 + ema12[i - 1]
        ema26[i] = (closes[i] - ema26[i - 1]) * mult26 + ema26[i - 1]
    macd = ema12 - ema26
    signal = np.zeros(n)
    mult9 = 2 / 10
    for i in range(1, n):
        signal[i] = (macd[i] - signal[i - 1]) * mult9 + signal[i - 1]
    features["macd"] = macd / closes
    features["macd_signal"] = signal / closes
    features["macd_hist"] = (macd - signal) / closes

    # Bollinger Bands position
    bb_pos = np.zeros(n)
    for i in range(19, n):
        sma20 = np.mean(closes[i - 19 : i + 1])
        std = np.std(closes[i - 19 : i + 1])
        upper = sma20 + 2 * std
        lower = sma20 - 2 * std
        bb_pos[i] = (closes[i] - lower) / (upper - lower + 1e-10)
    features["bb_position"] = bb_pos

    # ATR
    atr = np.zeros(n)
    for i in range(14, n):
        tr = max(
            highs[i] - lows[i],
            abs(highs[i] - closes[i - 1]),
            abs(lows[i] - closes[i - 1]),
        )
        atr[i] = (atr[i - 1] * 13 + tr) / 14
    features["atr_ratio"] = atr / closes

    # Volume
    vol_sma = np.zeros(n)
    for i in range(19, n):
        vol_sma[i] = np.mean(volumes[i - 19 : i + 1])
    features["volume_ratio"] = volumes / (vol_sma + 1e-10)

    # OBV
    obv = np.zeros(n)
    obv[0] = volumes[0]
    for i in range(1, n):
        if closes[i] > closes[i - 1]:
            obv[i] = obv[i - 1] + volumes[i]
        elif closes[i] < closes[i - 1]:
            obv[i] = obv[i - 1] - volumes[i]
        else:
            obv[i] = obv[i - 1]
    obv_change = np.zeros(n)
    obv_change[1:] = (obv[1:] - obv[:-1]) / (np.abs(obv[:-1]) + 1)
    features["obv_change"] = obv_change

    # Stochastic
    stoch = np.zeros(n)
    for i in range(13, n):
        high_max = np.max(highs[i - 13 : i + 1])
        low_min = np.min(lows[i - 13 : i + 1])
        if high_max > low_min:
            stoch[i] = 100 * (closes[i] - low_min) / (high_max - low_min)
    features["stoch_k"] = stoch

    # ADX (simplified)
    adx = np.zeros(n)
    for i in range(14, n):
        plus_dm = max(0, highs[i] - highs[i - 1])
        minus_dm = max(0, lows[i - 1] - lows[i])
        if atr[i] > 0:
            plus_di = 100 * plus_dm / atr[i]
            minus_di = 100 * minus_dm / atr[i]
            dx = 100 * abs(plus_di - minus_di) / (plus_di + minus_di + 1e-10)
            adx[i] = (adx[i - 1] * 13 + dx) / 14
    features["adx"] = adx

    # Volatility
    vol20 = np.zeros(n)
    for i in range(19, n):
        vol20[i] = np.std(closes[i - 19 : i + 1]) / np.mean(closes[i - 19 : i + 1])
    features["volatility_20"] = vol20

    # Momentum
    for period in [5, 10]:
        mom = np.zeros(n)
        mom[period:] = closes[period:] / closes[:-period] - 1
        features[f"momentum_{period}"] = mom

    return features


def create_training_data(candles, features, seq_len=128, horizon=24):
    """Create training samples"""
    feature_names = [
        "returns",
        "log_returns",
        "high_low_range",
        "close_position",
        "sma_5_ratio",
        "sma_10_ratio",
        "sma_20_ratio",
        "sma_50_ratio",
        "rsi_14",
        "rsi_7",
        "macd",
        "macd_signal",
        "macd_hist",
        "bb_position",
        "atr_ratio",
        "volume_ratio",
        "obv_change",
        "stoch_k",
        "adx",
        "volatility_20",
        "momentum_5",
        "momentum_10",
    ]

    n_valid = len(candles) - seq_len - horizon
    if n_valid <= 0:
        return None, None, None

    X = np.zeros((n_valid, seq_len, len(feature_names)), dtype=np.float32)
    y = np.zeros(n_valid, dtype=np.float32)

    closes = np.array([c["close"] for c in candles])

    for i in range(n_valid):
        for j, fname in enumerate(feature_names):
            X[i, :, j] = features[fname][i : i + seq_len]

        current = closes[i + seq_len - 1]
        future = closes[i + seq_len + horizon - 1]
        y[i] = (future - current) / current

    return X, y, feature_names


def main():
    print("=" * 60)
    print("Fetching Complete BTC Data (2017-Now)")
    print("=" * 60)

    # Fetch 1h data
    print("\n📈 Fetching 1h data...")
    candles_1h = fetch_all_klines("BTCUSDT", "1h", max_candles=80000)
    print(f"Total: {len(candles_1h)} candles")

    # Fetch 4h data
    print("\n📈 Fetching 4h data...")
    candles_4h = fetch_all_klines("BTCUSDT", "4h", max_candles=20000)
    print(f"Total: {len(candles_4h)} candles")

    # Fetch 1d data
    print("\n📈 Fetching 1d data...")
    candles_1d = fetch_all_klines("BTCUSDT", "1d", max_candles=5000)
    print(f"Total: {len(candles_1d)} candles")

    # Calculate indicators for 1h
    print("\n🔢 Calculating indicators for 1h...")
    features_1h = calculate_indicators(candles_1h)

    # Create training data
    print("\n📦 Creating training data...")
    X, y, feature_names = create_training_data(
        candles_1h, features_1h, seq_len=128, horizon=24
    )

    if X is None:
        print("Error: Not enough data")
        return

    print(f"X shape: {X.shape}")
    print(f"y shape: {y.shape}")

    # Normalize
    X_flat = X.reshape(-1, X.shape[-1])
    X_mean = np.nanmean(X_flat, axis=0)
    X_std = np.nanstd(X_flat, axis=0) + 1e-8

    # Replace NaN
    X = np.nan_to_num(X, nan=0)

    # Save
    output_path = "btc_complete_data.bin"
    print(f"\nSaving to {output_path}...")

    with open(output_path, "wb") as f:
        n_samples, seq_len, n_features = X.shape
        f.write(struct.pack("<I", n_samples))
        f.write(struct.pack("<I", seq_len))
        f.write(struct.pack("<I", n_features))
        X.tofile(f)
        y.tofile(f)
        X_mean.tofile(f)
        X_std.tofile(f)

    print(f"Saved: {n_samples} samples, {seq_len} seq, {n_features} features")
    print(f"File size: {os.path.getsize(output_path) / 1024 / 1024:.1f} MB")

    # Stats
    print(f"\nTarget stats:")
    print(f"  Mean: {y.mean() * 100:.4f}%")
    print(f"  Std: {y.std() * 100:.4f}%")
    print(f"  Min: {y.min() * 100:.4f}%")
    print(f"  Max: {y.max() * 100:.4f}%")

    # Save feature names
    with open("features_22.txt", "w") as f:
        f.write("\n".join(feature_names))
    print(f"\nFeatures: {feature_names}")


if __name__ == "__main__":
    main()
